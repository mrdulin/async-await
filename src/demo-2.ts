 ...
 * @param reject
 */
const handle = <T>(type: string, resolve: (value?: T | PromiseLike<T>) => void, reject: (reason?: any) => void) => {
  setTimeout(() => {
    console.log(type, Date.now());
    const error = ERROR_MAP[type];
    const apiRes: IApiSuccessResponse & { type: string } = {
 ...
 */
const A = () => new Promise((resolve, reject) => handle('A', resolve, reject));
const B = () => new Promise((resolve, reject) => handle('B', resolve, reject));
const C = () => new Promise((resolve, reject) => handle('C', resolve, reject));
      ...
      [A(), B(), C()].map(promise => {
        return (
          promise
            ...
            .then(
              (apiRes: IApiSuccessResponse & { type: string }): IState => {
                return {
                  [apiRes.type]: {
                    data: apiRes.content
                  }
